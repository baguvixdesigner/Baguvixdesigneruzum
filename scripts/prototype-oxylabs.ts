/**
 * ИТОГ РАССЛЕДОВАНИЯ (см. README): Oxylabs отклонён как источник для Taobao/1688.
 * У них нет "именных" источников с авто-парсингом для этих двух сайтов (только
 * generic source: "universal" — сырой HTML), а для Taobao даже сырой HTML с
 * JS-рендером и явным ожиданием не дал реальных данных товаров (asynchronous
 * подгрузка, которая просто не срабатывала). Переключились на специализированные
 * Apify-акторы — см. src/services/apifyChina.ts. Этот файл оставлен как рабочий
 * журнал расследования, не используется в проде.
 *
 ...
 * предположения в исходном src/services/oxylabs.ts (там был жёстко зашит
 * несуществующий именной source "taobao_search"/"1688_search").
 *
